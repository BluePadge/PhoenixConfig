...
// Next screen
keys.push(new Key('left', mashCmd, function () {
...
// Previous Screen
keys.push(new Key('right', mashCmd, function () {
   var window = getCurrentWindow();
   if (window === undefined) {
      return;
   }
...
keys.push(new Key('right', cmdCtrl, function () {
   var window = getCurrentWindow();
   if (window === undefined) {
      return;
   }
...
keys.push(new Key('left', cmdCtrl, function () {
   var window = getCurrentWindow();
   if (window === undefined) {
      return;
   }
   // if (window.screen() === window.screen().previous()) return;
   // if (window.screen().previous().flippedFrame().x == 0) {
   // return;
   // }
...
Key.on('right', ctrlShift, moveCurrentWindowToNextSpace)
...
Key.on('left', ctrlShift, moveCurrentWindowToPrewSpace)
...
// Window Maximize
keys.push(new Key('up', cmdCtrl, fullscreen));
keys.push(new Key('up', ctrlShift, fullscreen));
...
keys.push(new Key('g', ctrlShift, function () {
   var window = getCurrentWindow();
   if (window === undefined) {
      return;
   }
...
keys.push(new Key('down', ctrlShift, setWindowCentralWithSize))
keys.push(new Key('down', cmdCtrl, setWindowCentralWithSize))

function setWindowCentralWithSize() {
   var window = getCurrentWindow();
   if (window === undefined) {
      return;
   }
...
keys.push(new Key('tab', mash, function () {
   var window = Window.focused();
   if (!window) {
...
keys.push(new Key('down', shiftCmdCtrl, function () {
   var window = Window.focused();
   if (!window) {
      if (Window.recent().length == 0) return;
      Window.recent()[0].focus();
